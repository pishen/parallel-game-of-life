...
import info.pishen.util.AboutDialog;
import info.pishen.util.AlertBox;
import info.pishen.util.EasyFile;
import info.pishen.util.LineEnumerator;
import info.pishen.util.TextFileDialog;
...
public class StandaloneGameOfLife extends GameOfLife {
	...
	public void init(Frame parent) {
		...
		try {
			...
			DropTarget dt = new DropTarget(gameOfLifeCanvas,
	...
	class MyDropListener implements DropTargetListener {
		private final DataFlavor urlFlavor = new DataFlavor(
		...
		public void drop(DropTargetDropEvent event) {
			...
			if (event.isDataFlavorSupported(urlFlavor)) {
				...
				try {
					event.acceptDrop(DnDConstants.ACTION_COPY);
					Transferable trans = event.getTransferable();
					java.util.List list = (java.util.List) (trans
							.getTransferData(DataFlavor.javaFileListFlavor));
					File droppedFile = (File) list.get(0); // More than one file
															// -> get only first
															// file
	...
	class GameOfLifeGridIO {
		 ...
		 * 
		 * @param grid
		 *            grid to read/write files from/to
		 */
		 ...
		 * 
		 * @param filename
		 *            filename to load shape from, or null when no filename
		 *            given.
		 */
		public void openShape(String filename) {
			...
			try {
				...
				new AlertBox(appletFrame, "File not found",
						"Couldn't open this file.\n" + e.getMessage());
			} catch (IOException e) {
				new AlertBox(appletFrame, "File read error",
		 ...
		 * 
		 * @param url
		 *            URL pointing to GameOfLife-file
		 */
		public void openShape(URL url) {
			...
			try {
				...
				new AlertBox(appletFrame, "URL not found",
						"Couldn't open this URL.\n" + e.getMessage());
			} catch (IOException e) {
				new AlertBox(appletFrame, "URL read error",
		...
		public void setShape(Shape shape) {
			...
			if (shapeDim.width > gridDim.width
					|| shapeDim.height > gridDim.height) {
				...
				width = frameDim.width + cellSize
						* (shapeDim.width - gridDim.width);
				height = frameDim.height + cellSize
						* (shapeDim.height - gridDim.height);
				...
				if (width > screenDim.width || height > screenDim.height) {
					...
					// GameOfLifeControls.SIZE_SMALL corresponds with
					// GameOfLifeControls.SMALL
					int newCellSize = GameOfLifeControls.SIZE_SMALL;
					width = frameDim.width + newCellSize * shapeDim.width
							- cellSize * gridDim.width;
					height = frameDim.height + newCellSize * shapeDim.height
							- cellSize * gridDim.height;
					...
					// Now resize the window (and optionally set the new
					// cellSize)
      ...
      return new Shape( name, cellArray );
		...
		public void saveShape() {
			...
			try {
				...
				new AlertBox(appletFrame, "File error",
						"Couldn't open this file.\n" + e.getMessage());
			} catch (IOException e) {
				new AlertBox(appletFrame, "File error",
...
class AppletFrame extends Frame {
	...
	public AppletFrame(String title, StandaloneGameOfLife applet) {
		...
		// Java in Windows opens windows in the upper left corner, which is
		// ugly! Center instead.
		if (screenSize.width >= 640 && screenSize.height >= 480)
			setLocation((screenSize.width - 550) / 2,
	...
	private void showAboutDialog() {
		...
		String jvmProperties = "Java VM "
				+ properties.getProperty("java.version") + " from "
				+ properties.getProperty("java.vendor");
		Point p = getLocation();
		new AboutDialog(this, "About the Game of Life", new String[] {
	...
	private void showManualDialog() {
		Point p = getLocation();
		new TextFileDialog(this, "Game of Life Manual", "manual.txt", p.x + 60,
				p.y + 60);
	}
	...
	private void showLicenseDialog() {
		Point p = getLocation();
		new TextFileDialog(this, "Game of Life License", "license.txt",
				p.x + 60, p.y + 60);
	}
