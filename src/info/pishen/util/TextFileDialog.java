...
public class TextFileDialog extends Dialog {
	 ...
	 *            window title
	 * @param filename
	 *            filename to show (from jar-file)
	 */
	public TextFileDialog(Frame parent, String title, String filename,
			int posX, int posY) {
		super(parent, title);
		String manualText;
		...
		EasyFile file = new EasyFile(this.getClass().getResourceAsStream(
				filename));
		try {
		...
		TextArea manualTextArea = new TextArea(manualText, 20, 50,
				TextArea.SCROLLBARS_VERTICAL_ONLY);
		manualTextArea.setBackground(Color.white); // improve readability
		manualTextArea.setForeground(Color.black); // we don't want white text
